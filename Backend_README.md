...
flowchart TB
    ...
    FE -->|REST + WebSocket| ING["Kubernetes Service<br/>(NodePort)"]

    subgraph K8S["Kubernetes Cluster"]
        ING --> P1["Backend Pod 1"]
        ING --> P2["Backend Pod 2"]
        ING -.->|autoscaled 2-8| PN["Backend Pod N"]
        HPA["HPA Controller<br/>(CPU target: 50%)"] -.->|scales| P1
        HPA -.->|scales| P2
        HPA -.->|scales| PN
        CM["ConfigMap<br/>(PORT, EMAIL_USER)"] --> P1
        SEC["Secret<br/>(MONGO_URI, JWT_SECRET, EMAIL_PASS)"] --> P1
    end

    P1 -->|TLS| ATLAS[("MongoDB Atlas<br/>(M0 Free Tier)")]
    P2 -->|TLS| ATLAS
    P1 --> S3[("AWS S3<br/>File Storage")]
    P1 -->|SMTP| EMAIL["Email Service"]

    style K8S fill:#eef4fb,stroke:#144c6e
