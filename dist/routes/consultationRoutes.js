...
const consultationController_2 = require("../controllers/consultationController");
const uploadMiddleware_1 = require("../middleware/uploadMiddleware");
const consultationController_3 = require("../controllers/consultationController");
const consultationController_4 = require("../controllers/consultationController");
const consultationController_5 = require("../controllers/consultationController");
const router = express_1.default.Router();
router.get('/my-requests', authMiddleware_1.protect, consultationController_1.getMyConsultations);
router.post('/submit', authMiddleware_1.protect, consultationController_1.submitConsultation);
router.get('/admin/request/:id', authMiddleware_1.protect, authMiddleware_1.admin, async (req, res) => {
    const request = await Consultation_1.Consultation.findById(req.params.id).populate('user', 'fullName email');
    res.json(request);
});
router.post('/submit-data', authMiddleware_1.protect, async (req, res) => {
    try {
        const { answers } = req.body;
        if (!answers.industry || !answers.revenue || !answers.currentSystem || !answers.serviceNeed || !answers.employeeCount) {
            return res.status(400).json({ message: "Please answer all questions before submitting." });
        }
        const newConsultation = await Consultation_1.Consultation.create({
            user: req.user._id,
            answers,
            status: 'pending'
        });
...
router.get('/admin/all', authMiddleware_1.protect, authMiddleware_1.admin, async (req, res) => {
    try {
        const allRequests = await Consultation_1.Consultation.find()
            .populate('user', 'fullName email')
            .sort({ submittedAt: -1 });
        res.status(200).json(allRequests);
    }
...
router.put('/admin/update/:id', authMiddleware_1.protect, authMiddleware_1.admin, async (req, res) => {
    try {
        const { status, adminComment } = req.body;
        const allowedStatuses = ['pending', 'accepted', 'rejected', 'ongoing'];
        if (!allowedStatuses.includes(status)) {
        ...
        const updatedRequest = await Consultation_1.Consultation.findByIdAndUpdate(req.params.id, {
            status,
            adminComment: adminComment || "No comment provided by admin."
        }, { new: true }).populate('user', 'fullName email');
        if (!updatedRequest) {
            return res.status(404).json({ message: "Request not found" });
        }
        res.status(200).json(updatedRequest);
...
router.delete('/admin/delete/:id', authMiddleware_1.protect, authMiddleware_1.admin, consultationController_2.deleteRequestAdmin);
router.post('/upload-docs/:id', authMiddleware_1.protect, uploadMiddleware_1.upload.array('documents', 5), consultationController_3.uploadConsultationDocs);
router.delete('/:id/document', authMiddleware_1.protect, consultationController_4.deleteDocument);
router.put('/admin/update-status/:id', authMiddleware_1.protect, authMiddleware_1.admin, consultationController_5.updateRequestStatus);
