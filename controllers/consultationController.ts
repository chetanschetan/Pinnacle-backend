...
// // ⚠️ Top wala global S3 client hata dena agar wahan hai, aur isko function ke andar use karo:

// // Upload Consultation Documents (On-the-fly S3 client initialization)
...
//     // Direct function ke andar client banao taaki env variables 100% milein
...
// Global S3 Client (Safe & Robust Initialization)
const s3 = new S3Client({
  region: (process.env.AWS_REGION || 'ap-south-1').trim(),
  credentials: {
    accessKeyId: String(process.env.AWS_ACCESS_KEY_ID || '').trim().replace(/['"]+/g, ''),
...
// Upload Consultation Documents (Using global robust S3 client)
export const uploadConsultationDocs = async (req: any, res: Response) => {
  try {
    const { id } = req.params;
    if (!req.files || req.files.length === 0) {
    ...
    const bucketName = process.env.AWS_BUCKET_NAME || 'pinnacle-storage-2026';
    const region = process.env.AWS_REGION || 'ap-south-1';
...
export const deleteDocument = async (req: any, res: Response) => {
  try {
    ...
    if (consultation.documents.length < initialCount) {
      ...
      // Delete from AWS S3 bucket using global s3 client
      try {
        const urlObj = new URL(filePath);
        const s3Key = decodeURIComponent(urlObj.pathname.substring(1));
