...
// Helper function to get a fresh S3 client every time with active .env variables
const getS3Client = () => {
  return new S3Client({
...
// Upload Consultation Documents (Using dynamic fresh S3 client)
...
// Delete Document (Using dynamic fresh S3 client)
export const deleteDocument = async (req: any, res: Response) => {
  try {
    ...
    if (consultation.documents.length < initialCount) {
      ...
      // Delete from AWS S3 bucket using dynamic s3 client
      try {
        const s3Client = getS3Client();
