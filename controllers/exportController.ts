...
// =============================================================
export const exportAllDataExcel = async (req: Request, res: Response) => {
  try {
    const users = await User.find({}).lean();
    const consultations = await Consultation.find({}).populate('user', 'fullName email').sort({ createdAt: -1 }).lean();
...
// =============================================================
export const exportUsersPDF = async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      const userConsultations = await Consultation.find({ user: u._id }).sort({ createdAt: -1 }).lean();
      doc.moveDown(0.5);
      doc.fontSize(10).fillColor('#0F172A').text(`Consultation Requests (${userConsultations.length}):`);

      if (userConsultations.length > 0) {
        userConsultations.forEach((c: any, cIdx: number) => {
...
// =============================================================
export const exportSingleUserExcel = async (req: Request, res: Response) => {
  try {
    ...
    if (consultations.length === 0) {
      ...
      consultations.forEach((c: any, index: number) => {
        ...
        if (c.documents && c.documents.length > 0) {
          c.documents.forEach((docItem: any, dIdx: number) => {
...
// =============================================================
export const exportSingleUserPDF = async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
